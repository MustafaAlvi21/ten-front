...
export default function NavbarSidebar({connectMetamask, user}){

  const [connect, set_connect] = useState("Connect Wallet 123")
  // const connect = "Connect Wallet 123"
...
async function checkNetwork() {
  ...
  return(
    <nav className="navbar navbar-expand navbar-light navbar-bg" style={{ border: "1px solid red"}}>
      ...
      <div className="navbar-collapse collapse">
        <ul className="navbar-nav navbar-align" style={{display: 'flex', alignItems: 'baseline'}}>
          ...
          &nbsp; &nbsp;
          <li>
              <button className="btn btn-primary px-3 color-yellow vimlandnav" onClick={()=>checkNetwork()} style={{WebkitBoxAlign: 'center', alignItems: 'center', border: 0, borderRadius: 16, boxShadow: 'rgb(14 14 44 / 40%) 0px -1px 0px 0px inset', cursor: 'pointer', display: 'inline-flex', fontFamily: 'inherit', fontSize: 16, fontWeight: 600, WebkitBoxPack: 'center', justifyContent: 'center', letterSpacing: '0.03em', lineHeight: 1, opacity: 1, outline: 0, transition: 'background-color 0.2s ease 0s, opacity 0.2s ease 0s', height: 32, padding: '0px 16px', backgroundColor: 'rgb(31, 199, 212)', color: 'white'}} target="_blank"> { connect } </button>

              {/* <button
              onClick={() => {if (Object.keys(user).length === 0){load_web3()}}}
